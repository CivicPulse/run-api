...
async def test_get_canvassing_canvassers_success() -> None:
    ...
    try:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            resp = await c.get(f"/api/v1/campaigns/{CAMPAIGN_ID}/dashboard/canvassing/canvassers")
        assert resp.status_code == 200
    finally:
        _restore_dashboard_services(m, orig)
...
async def test_get_canvassing_turfs_success() -> None:
    ...
    try:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            resp = await c.get(f"/api/v1/campaigns/{CAMPAIGN_ID}/dashboard/canvassing/turfs")
        assert resp.status_code == 200
    finally:
        _restore_dashboard_services(m, orig)
...
async def test_get_phone_banking_summary_success() -> None:
    ...
    try:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            resp = await c.get(f"/api/v1/campaigns/{CAMPAIGN_ID}/dashboard/phone-banking")
        assert resp.status_code == 200
...
async def test_get_phone_banking_sessions_success() -> None:
    ...
    try:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            resp = await c.get(f"/api/v1/campaigns/{CAMPAIGN_ID}/dashboard/phone-banking/sessions")
        assert resp.status_code == 200
    finally:
        _restore_dashboard_services(m, orig)
...
async def test_get_phone_banking_callers_success() -> None:
    ...
    try:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            resp = await c.get(f"/api/v1/campaigns/{CAMPAIGN_ID}/dashboard/phone-banking/callers")
        assert resp.status_code == 200
    finally:
        _restore_dashboard_services(m, orig)
...
async def test_get_volunteers_list_success() -> None:
    ...
    try:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            resp = await c.get(f"/api/v1/campaigns/{CAMPAIGN_ID}/dashboard/volunteers/volunteers")
        assert resp.status_code == 200
    finally:
        _restore_dashboard_services(m, orig)
