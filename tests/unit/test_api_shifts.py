...
START = datetime(2026, 4, 5, 9, 0, 0, tzinfo=UTC)
END = datetime(2026, 4, 5, 12, 0, 0, tzinfo=UTC)
...
async def test_cancel_self_signup_is_idempotent_when_not_signed_up() -> None:
    db = AsyncMock()
    db.commit = AsyncMock()
    svc = AsyncMock()
    svc.cancel_signup.side_effect = ValueError(
        f"Volunteer {VOLUNTEER_ID} not signed up for shift {SHIFT_ID}"
    )

    user = _make_user(user_id="volunteer-1", role=CampaignRole.VOLUNTEER)
    app = _override_app(user, db)
    _setup_role_resolution(db, "volunteer")
    ...
    campaigns_row = MagicMock()
    campaigns_row.scalars.return_value.all.return_value = []
    volunteer_row = MagicMock()
    volunteer_row.scalar_one_or_none.return_value = SimpleNamespace(id=VOLUNTEER_ID)
    db.execute = AsyncMock(
        side_effect=[
            user_row,
            org_row,
    ...
    try:
        async with AsyncClient(
            ...
            resp = await c.delete(
                f"/api/v1/campaigns/{CAMPAIGN_ID}/shifts/{SHIFT_ID}/signup"
            )
        assert resp.status_code == 204
    finally:
        m._shift_service = orig
