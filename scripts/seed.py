...
async def main() -> None:  # noqa: C901, PLR0915
    ...
    async with session_factory() as session, session.begin():
        ...
        survey = SurveyScript(
            ...
            description=(
                "Standard door-to-door and phone bank survey for voter engagement"
            ),
            status="active",
            created_by=user_owner_id,
            created_at=NOW,
        ...
        def _voters_for(nb: str) -> list[Voter]:
            return [v for v in voter_records if v.extra_data.get("neighborhood") == nb]
        ...
        remaining = [
            v
            for v in voter_records
            if v not in ingleside_voters and v not in vineville_voters
        ]
        vineville_for_list = (
            vineville_voters + remaining[: max(0, 12 - len(vineville_voters))]
        )
        ...
        vol_data = [
            ("Aisha", "Brooks", "active", ["canvassing", "voter_registration"], None),
            ("Carlos", "Hernandez", "active", ["phone_banking", "data_entry"], None),
            (
                "Destiny",
                "Washington",
                "active",
            ...
            ("Enrique", "Morales", "active", ["social_media", "graphic_design"], None),
            (
                "Fatima",
                "Ali",
                "active",
            ...
            ("Isabella", "Osei", "active", ["data_entry", "voter_registration"], None),
            (
                "Jamal",
                "Patterson",
                "active",
                ["canvassing", "event_setup"],
                user_canvasser2_id,
            ),
            (
                "Keisha",
            ...
            (
                ...
                "O'Brien",
                "active",
                ["canvassing", "phone_banking", "driving"],
                None,
            ),
            (
                "Maya",
                "Pham",
                "active",
                ["translation", "data_entry", "phone_banking"],
                None,
            ),
            ("Noah", "Cooper", "inactive", ["canvassing"], None),
            ("Olivia", "Reed", "inactive", ["phone_banking", "data_entry"], None),
            ("Pedro", "Santos", "inactive", ["event_setup"], None),
            ("Quinn", "Barrett", "inactive", ["social_media", "graphic_design"], None),
            ("Rosa", "Delgado", "pending", ["canvassing", "translation"], None),
            ("Samuel", "Foster", "pending", ["phone_banking"], None),
            ("Tanya", "Griffin", "pending", ["voter_registration"], None),
            ("Victor", "Huang", "pending", ["data_entry", "fundraising"], None),
        ]
        ...
        for idx, (first, last, status, skills, uid) in enumerate(vol_data):
            ...
            vol = Volunteer(
                ...
                emergency_contact_phone=(
                    f"478-555-{random.randint(1000, 9999)}" if has_ec else None
                ),
                created_by=user_owner_id,
        ...
        await session.flush()
        active_vols = [v for v in volunteer_records if v.status == "active"]
        print(
            f"  Created {len(volunteer_records)} volunteers ({len(active_vols)} active)"
        )
        ...
        for i, v in enumerate(evening_voters):
            ...
            cl_entries.append(
                CallListEntry(
                    ...
                    claimed_by=(user_volunteer_id if status != "available" else None),
                    claimed_at=(
                        NOW - timedelta(hours=2) if status != "available" else None
                    ),
                    last_attempt_at=(
                        NOW - timedelta(hours=1) if status != "available" else None
                    ),
                )
            )
        ...
        for i, v in enumerate(morning_voters):
            ...
            cl_entries.append(
                CallListEntry(
                    ...
                    claimed_at=NOW - timedelta(days=random.randint(3, 10)),
                    last_attempt_at=NOW - timedelta(days=random.randint(3, 7)),
                )
            )
        ...
        await session.flush()
        print(f"  Created {len(call_lists)} call lists with {len(cl_entries)} entries")
        ...
        shift_defs = [
            ("Sat Canvass - Ingleside", "canvassing", "completed", -14, 4, 8, 0),
            ("Sat Canvass - Vineville", "canvassing", "completed", -12, 4, 6, 1),
            (
                "Wed Phone Bank - Evening",
                "phone_banking",
                "completed",
                -10,
                3,
            ...
            ("Sat Canvass - Pleasant Hill", "canvassing", "completed", -7, 4, 8, 2),
            ("Sat Canvass - Downtown", "canvassing", "completed", -5, 4, 6, 3),
            ("Volunteer Orientation", "general", "completed", -3, 2, 15, None),
            ("Tue Canvass - Bellevue", "canvassing", "active", 0, 4, 8, 4),
            ("Thu Phone Bank - Morning", "phone_banking", "scheduled", 2, 3, 10, None),
            ("Sat Canvass Blitz", "canvassing", "scheduled", 4, 6, 20, 0),
            ("Cancelled Rain Date", "canvassing", "cancelled", -1, 4, 8, 2),
        ]
        ...
        for (
            ...
            shift = Shift(
                ...
                street=(f"{random.randint(100, 999)} {random.choice(STREET_NAMES)}"),
                city="Macon",
                state="GA",
                zip_code="31201",
                latitude=32.840 + random.uniform(-0.01, 0.01),
                longitude=-83.635 + random.uniform(-0.01, 0.01),
                turf_id=(turf_records[turf_idx].id if turf_idx is not None else None),
                phone_bank_session_id=(
                    pb_ids[0]
                    if stype == "phone_banking" and status == "completed"
                    else None
                ),
                created_by=random.choice([user_owner_id, user_manager_id]),
        ...
        for shift_idx, shift in enumerate(shift_records):
            ...
            num_signups = min(shift.max_volunteers, random.randint(3, 8))
            chosen_vols = random.sample(active_vols, min(num_signups, len(active_vols)))
            ...
            for vi, vol in enumerate(chosen_vols):
                if shift.status == "completed":
                    ...
                    sv_status = (
                        "checked_in" if vi < len(chosen_vols) - 1 else "signed_up"
                    )
                    ...
                    sv_status = (
                        "waitlisted" if vi >= shift.max_volunteers - 2 else "signed_up"
                    )
                ...
                if sv_status == "checked_out":
                    ...
                    check_out = shift.end_at - timedelta(minutes=random.randint(0, 30))
                elif sv_status == "checked_in":
                    check_in = shift.start_at + timedelta(minutes=random.randint(0, 15))
            ...
            interactions.append(
                VoterInteraction(
                    ...
                    payload={
                        ...
                        "duration_seconds": (
                            random.randint(30, 300) if result == "answered" else 0
                        ),
                    },
                    created_by=caller,
                    created_at=NOW
        ...
        def _add_vtm(vid, tag_idx):
            ...
            if pair not in assigned_pairs:
                vtm_records.append(VoterTagMember(voter_id=vid, tag_id=tid))
        ...
        for vol in active_vols:
            ...
            avail_records.append(
                VolunteerAvailability(
                    id=uuid.uuid4(),
                    volunteer_id=vol.id,
                    start_at=NOW + timedelta(days=sat_offset, hours=9 - NOW.hour),
                    end_at=NOW + timedelta(days=sat_offset, hours=12 - NOW.hour),
                )
            )
        ...
        await session.flush()
        print(f"  Created {len(avail_records)} volunteer availability windows")
        ...
        for i in range(20):
            ...
            dnc_records.append(
                DoNotCallEntry(
                    ...
                    added_by=random.choice([user_owner_id, user_manager_id]),
    ...
    print(
        f"  Volunteer tags: {len(vol_tag_records)} ({len(vol_tm_records)} assignments)"
    )
