...
## Scope Decision (2026-04-10, user-approved)

Post-unblock scope narrowed to **Option D — Hybrid, highest-signal clusters**. See `106-CONTEXT.md` §D-15 for the full locked-in decision.

**IN scope for 106-02/03/04/05:** pytest (2 fails), vitest shifts cluster (34 of 65 via one mechanical fix) + vitest remainder (~31), Playwright rbac cluster (44, likely shared-helper fix), Playwright pitfall-5 deletes (~11 across `field-mode.volunteer` / `map-interactions` / `walk-lists` / `useCanvassing.test.ts` → phases 107/109), D-10 skip-marker audit (6 unjustified in rbac.*, handled by the rbac cluster work).

**DEFERRED** to `.planning/todos/pending/106-phase-verify-cluster-triage.md` (target v1.19): phase-verify cluster (~51 fails across 9 specs) + misc deferred (~29-40 fails across ~20 specs). Before phase 106-05 exit gate, every deferred test gets a D-10-compliant justified `test.skip` referencing that todo file so the green bar doesn't break.

**D-12 exit gate for 106-05** is adjusted: Playwright green-bar = the IN-scope specs pass on 2 consecutive `run-e2e.sh` runs; deferred specs carry justified skips. The gate verifies trustworthiness for the IN-scope surface, not the entire 396-test suite.

Plan executors for 106-02/03/04/05 MUST read D-15 in `106-CONTEXT.md` AND this footer before starting — the phase appetite has changed from the original "triage everything" framing.

---

*Generated by phase 106-01 baseline capture. Raw suite output is in `artifacts/*-baseline.txt` (gitignored). Env-unblock fixes tracked in commits `ae63e34` and `2e830c6`. Baseline re-capture in `0df3298`. Scope decision D-15 recorded under user approval 2026-04-10 after the re-evaluated scope-explosion checkpoint.*
