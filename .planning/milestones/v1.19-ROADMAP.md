# Roadmap Archive: v1.19 Invite Onboarding

**Archived:** 2026-04-23
**Status:** ABANDONED — pivoted to v1.20 Native Auth Rebuild

Phase 111 `urlTemplate` deep-link spike **FAILED**: ZITADEL v4.10.1 bundles only the legacy Go-templates login UI and does not honor the `urlTemplate` parameter. Plans 111-02 through 111-06 were blocked; phases 112-115 were never planned.

After sizing analysis showed Option C (non-ROPC ZITADEL workaround) was equivalent effort to a native rebuild, the milestone pivoted on 2026-04-23 to v1.20 DIY auth (fastapi-users + cookie sessions). See:

- `.planning/notes/decision-drop-zitadel-diy-auth.md` — full decision record
- `.planning/seeds/SEED-003-revisit-zitadel-when-sso-needed.md` — tripwires for revisiting ZITADEL
- `.planning/milestones/v1.19-phases/111-urltemplate-spike-zitadel-service-surface/` — preserved spike artifacts including verdict

Research from v1.19 (invite-flow design, pitfall analysis) was preserved and partially reused in v1.20 Phase 116 scoping.
