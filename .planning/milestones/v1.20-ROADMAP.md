# Roadmap Archive: v1.20 Native Auth Rebuild & Invite Onboarding

**Archived:** 2026-04-23
**Status:** SHIPPED out-of-flow 2026-04-23 — native auth live in prod via PR #32 (`f99e2cae`); SEED-002 partially delivered

**Execution note:** Phases 112-119 as scoped below were NOT executed sequentially through the GSD phase/plan flow. The core auth rewrite (Phases 113-115 + 116 partial + 117 partial) landed as a single merged PR branch to speed delivery. Phase 112 (SEED-002 continuous verification) partially landed: `.pre-commit-config.yaml`, `scripts/analyze-e2e-trend.sh`, and `scripts/doctor.sh` are in place; remaining SEED-002 work (nightly CI, seed002 gate-check script, self-coverage smokes) carries forward as tech debt. Phase 118 (ZITADEL tear-out) intentionally deferred pending production soak. Phase 119 (session lifecycle admin controls) deferred to a future milestone.

See MILESTONES.md for the actual shipped deliverables (extracted from git log since the 2026-04-23 pivot).
